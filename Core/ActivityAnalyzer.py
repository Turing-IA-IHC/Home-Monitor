...
class ActivityAnalyzer(Component):
    ...
    limit:int = -1
    lastTime:float = -1
    loaderChannelsThread:Process = None
    ...
    def start(self):
        ...
        while self.running:
            ...
            #    if self.LoaderChannelsThread == None or not self.LoaderChannelsThread.is_alive():
            #        self.loadChannels()
            #except:
            #    dataE = Data()
    ...
    def loadChannels(self):
        ...
        loaderChannel = LoaderChannel(self.CONFIG, self.COMMPOOL)
        loaderChannel.ANALYZER_PATH = self.ME_PATH
        self.queueMessages = Queue()
        self.loaderChannelsThread = Process(target=loaderChannel.start, args=(self.queueMessages,))
        self.loaderChannelsThread.start()
        del loaderChannel
